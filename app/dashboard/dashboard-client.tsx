...
type TabId = 'bandeja' | 'empresa' | 'facturacion';
...
export function DashboardClient({
  ...
  async function sync(provider: 'google' | 'trustpilot' | 'places', tenantId: string) {
    setSyncing(`${provider}-${tenantId}`);
    const isTrustpilot = provider === 'trustpilot';
    const endpoint = isTrustpilot ? '/api/integrations/trustpilot/sync' : '/api/integrations/google/sync';
    const payload = isTrustpilot
      ? { tenantId }
