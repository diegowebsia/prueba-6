...
import { negativeReviewAlert, sendWhatsappForTenant } from '@/lib/whatsapp';
import { systemLog } from '@/lib/logger';

const Body = z.object({
  api_key: z.string().min(10),
  source: z.enum(['google', 'trustpilot', 'facebook', 'manual']),
...
export async function POST(req: Request) {
  ...
  // Alerta ≤3★ al móvil del negocio (feature `whatsappAlerts`, planes Pro/Business).
  if (input.rating <= 3 && settings.whatsapp_to) {
    const alert = await sendWhatsappForTenant(
    ...
    alertSent = alert.ok
      ? { ok: true }
      : { ok: false, error: alert.error, code: String(alert.code) };
    if (!alert.ok) {
      await systemLog('warn', 'whatsapp.alert', alert.error, { tenantId: tenant.id, code: alert.code });
    }
  }
  ...
  if (input.customer_phone && input.order_id) {
    const { handleDeliveredOrder } = await import('@/lib/store');
    const delivered = await handleDeliveredOrder(admin, tenant as any, {
      orderId: input.order_id,
      customerName: input.author_name,
      customerPhone: input.customer_phone,
      provider: 'store',
    });
    orderMessage = delivered.ok
      ? { ok: true }
      : { ok: false, error: String((delivered.body as any)?.error ?? 'No enviado') };
  }
