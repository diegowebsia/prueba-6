...
/** ¿Hay `pg` + `DATABASE_URL`? (la app funciona igual sin ellos) */
export function isDbPoolConfigured(): boolean {
  return dbPoolConfig().configured && Boolean(loadPg());
}
...
/** Ejecuta varias sentencias en una transacción (rollback automático). */
export async function transaction<T>(
  fn: (client: PoolClient) => Promise<T>,
): Promise<{ ok: true; value: T } | { ok: false; error: string }> {
  const p = getDbPool();
  if (!p) return { ok: false, error: poolError ?? 'DATABASE_URL no configurada.' };
  ...
  try {
    ...
    return { ok: false, error: e?.message ?? 'Error en la transacción.' };
  } finally {
    client?.release();
  }
}
...
/** Aplica la purga del plan a una empresa (equivalente al cron de la migración). */
export async function purgeTenantDb(tenantId: string): Promise<QueryResult<{ reviews_purged: number; audit_purged: number }>> {
  return query('select * from public.purge_tenant($1::uuid)', [tenantId]);
}

/** Poda global (opiniones, auditoría y `system_logs`) — uso interno/cron. */
export async function purgeAllTenantsDb(): Promise<QueryResult<{ purge_all_tenants: number }>> {
  return query('select public.purge_all_tenants()');
}
