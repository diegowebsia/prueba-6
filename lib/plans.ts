/**
 * ============================================================
 * ReviewFlow AI — Catálogo comercial de planes (v3.9.0)
 * ============================================================
 ...
 *      sincronizaciones automáticas con Google/Trustpilot.
...
/** Todos los planes del catálogo son de pago (no existe plan gratuito). */
export const PAID_PLAN_IDS: PlanId[] = ['pro', 'business'];
export const PAID_PLANS: PlanDefinition[] = PAID_PLAN_IDS.map((id) => PLANS[id]);
...
/**
 * Estrategia de purga/archivado documentada (aplicada por `lib/usage.ts`):
 *  1. Las opiniones que exceden `reviewsStored` se eliminan de más antigua a
 *     más reciente (la copia agregada queda en `usage_counters`).
 *  2. El ledger `quota_events` conserva las `auditRows` filas más recientes
 *     por empresa: es auditoría operativa, no un histórico eterno.
 *  3. `system_logs` (tabla compartida) se poda globalmente cada hora con la
 *     retención más larga de la instancia (365 días) para que no crezca sin
 *     límite sin borrar el histórico de los planes superiores.
 *  4. Las conexiones nunca se purgan: si se alcanza `integrations`, la API
 *     rechaza la nueva conexión con un mensaje claro.
 */
export const PURGE_STRATEGY = {
  reviews: 'purga de las opiniones más antiguas al superar `limits.reviewsStored`',
  audit: 'purga del ledger `quota_events` por antigüedad (`limits.logRetentionDays`) y por volumen (`limits.auditRows`)',
  ai: 'purga de `ai_interactions` por antigüedad y por volumen (`limits.aiRows`)',
  logs: 'purga global horaria de `system_logs` con retención de 365 días',
  integrations: 'rechazo de nuevas conexiones al llegar a `limits.integrations`',
} as const;
