...
export const DEFAULT_OPENAI_MODEL = DEFAULT_AI_MODEL;
...
export const modelPricing = aiModelPricing;
 ...
 */
export function checkTenantRateLimit(tenantId: string): RateVerdict {
  const now = Date.now();
  sweep(now);
...
export function openAiConcurrency(): { inFlight: number; max: number } {
  return { inFlight, max: OPENAI_MAX_CONCURRENCY };
}
