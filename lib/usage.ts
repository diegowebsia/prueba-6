...
export async function canConsume(
  ...
  const check = await checkQuota(admin, tenantId);
  return check.hasAccess && check.metrics[metric].used + amount <= check.metrics[metric].quota;
}
...
export function quotaExceeded(check: QuotaCheck) {
  const metric = check.blockedBy && check.blockedBy !== 'storage' ? check.blockedBy : 'requests';
  return {
    error: quotaExhaustedMessage(check, metric, 1),
    code: 'quota_exhausted' as const,
    quota: publicQuota(check),
    addonUrl: `/api/stripe/addon?pack=${suggestPack(check) ?? 'extra_requests_1000'}`,
    upgrade: upgradeHints(check),
  };
}
...
/** Coste medio por fila en KB (expuesto para las guías y el panel). */
export const STORAGE_ROW_KB = ROW_KB;
